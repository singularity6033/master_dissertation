...
# for i in range(2):#     axs[0, i].set_title(titles[i], fontproperties=font_name)
...
for i in range(3, 4):
    ...
    for j in range(13):
        p = os.path.sep.join([file_path, names[i] + '_fpl_tf2', 'logs_frcnn_voc', 'w' + str(j + 1),
                              'loss', 'epoch_val_loss.txt'])
        loss = []
        for k in range(50):
            line = linecache.getline(p, k + 1)
...
p = os.path.sep.join([file_path, 'sgd_tf2_ndn_best', 'logs_frcnn_voc', 'vgg16_cifar100', 'loss', 'epoch_val_loss.txt'])
line = linecache.getline(p, 1)
loss = []
for k in range(50):
    line = linecache.getline(p, k + 1)
...
axs[1].grid()
# fig.show()
fig.savefig('./plot/vgg16_cifar100_frcnn_loss.png', dpi=300)
